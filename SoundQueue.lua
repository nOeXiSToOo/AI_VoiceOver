...
function SoundQueue:AddSoundToQueue(soundData)
    ...
    if soundData.event == Enums.SoundEvent.Tip then
        -- Para consejos, asignamos un ID único
        self.soundIdCounter = self.soundIdCounter + 1
        soundData.id = self.soundIdCounter
        
        if not Utils:TestTip(soundData) then
            Debug:Print(format("Tip sound failed to load: %s", soundData.filePath or ""))
            return
        end
        ...
        -- Si la cola solo contiene un sonido, lo reproducimos inmediatamente
    ...
    -- Comportamiento normal para otros tipos de eventos

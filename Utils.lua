...
function Utils:PlaySound(soundData)
    -- Manejo especial para consejos
    if soundData.event == Enums.SoundEvent.Tip then
        local handle = PlaySoundFile(soundData.filePath, Addon.db.profile.Audio.SoundChannel)
...
function Utils:TestTip(soundData)
    -- Verificar si el archivo existe sin reproducirlo
    local fileExists = false
    -- En versiones más recientes de WoW podemos usar C_FileSystem si está disponible
    if C_FileSystem and C_FileSystem.FileExists then
        fileExists = C_FileSystem.FileExists(soundData.filePath)
    else
        -- En versiones antiguas, asumimos que existe y dejamos que el sistema
        -- de reproducción maneje el error si no existe
        fileExists = true
    end
    return fileExists
end
